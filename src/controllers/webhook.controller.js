...
async function verifyWebhook(req, res) {
    ...
    console.log('[Webhook] Verify attempt:', { mode, token }); // add this line
  
    if (mode === 'subscribe' && token === process.env.META_VERIFY_TOKEN) {
      console.log('[Webhook] Verified by Meta ✅');
      return res.status(200).send(challenge);
    }
    console.log('[Webhook] FAILED - token mismatch. Expected:', process.env.META_VERIFY_TOKEN);
...
async function handleWebhook(req, res){
    // Always acknowledge immediately ( Meta exprects < 5s response )
    ...
    try{
        ...
        const business = await prisma.business.findUnique({
            where: {
                waPhoneId: phoneNumberId, isActive: true
            },
        });
...
async function createOrUpdateOrder(bsuiness, customer, conversation, orderData){
